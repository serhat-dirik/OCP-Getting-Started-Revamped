# stacks/resilience — M21 Resilience, Multi-Cluster & DR
...
| `pp-service-interconnect` (`components/service-interconnect`) | `[ADD-ON]` | **off (commented out)** | Skupper v2 → cross-site L7 connectivity for M21's optional section |
...
- Synthetic node labels `workshop.redhat.com/zone={a,b,c}` for the M21 chaos-drill zone-spread narrative
...
namespaces (`gitops/workshop-config/templates/per-user-resilience.yaml`) and the M21 entry state.
