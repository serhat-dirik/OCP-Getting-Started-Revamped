...
1. **Verify, never recall.** Product versions / UI paths / CR fields — and AI model names / endpoint contracts (a wrong model default once propagated through 5 files unverified; per-user MaaS keys are model-scoped) — come from `versions.yaml` (fresh <60 days) or get re-verified on docs.redhat.com / a live cluster — never from model memory. RBAC capability claims too: prove absence with `oc auth can-i` as the real identity, never with repo greps (stock cluster roles carry more than anyone remembers — the admin role's serviceaccount-impersonation grant falsified a SEV1). Mark unverifiable steps `// TODO(verify-on-cluster)`; zero TODOs at Definition of Done.
