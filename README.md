...
| A | M02 | Ways to Build & Deliver Apps | Build the claims service from Git with Source-to-Image, then rebuild it with a Dockerfile to see why you'd choose one over the other. Deploy a polyglot Node.js service and a database from the catalog, then wire the claims service to it. | ~3-4 min | 30 min |
...
| B | M10 | GitOps Fundamentals | Change the cluster by hand and watch it not stick, then create your first Argo CD Application and let it sync from Git instead. Cause drift on purpose, watch Argo CD self-heal it, then promote a git-driven change to stage. | ~2-4 min | 40 min |
| B | M11 | GitOps at Scale & Progressive Delivery | Replace a pile of hand-authored Argo CD Applications with one ApplicationSet that generates them from a template, then order a database migration with sync waves and hooks. Ship a change to production as a canary, and watch an automated analysis roll it back on its own when it fails. | ~5-8 min | 40 min |
...
| D | M19 | Service Mesh 3 & Advanced Gateways | Enroll the claims chain into the service mesh and prove mutual TLS by watching a plaintext client get refused, then watch live traffic in the Kiali graph. Shift traffic between service versions, inject faults and a circuit breaker, and lock down who may call whom with an AuthorizationPolicy. | ~4-6 min | 40 min |
...
| D | M21 | Eventing Deep-Dive & Serverless Workflows | Route a real CloudEvent through a broker to wake a consumer from zero, then filter delivery by attribute across several independent triggers. Break a consumer on purpose and watch Knative retry it and then dead-letter the event, reading exactly why it failed. | ~4-6 min | 40 min |
...
| D | M24 | Agentic AI on OpenShift | Rewrite the weak system prompt a running AI agent shipped with and watch it stop describing a tool call and start making one. Inspect its MCP tools, ground an answer in policy documents with RAG, then build and register a tool server of your own. Make the agent state a claim status no tool ever returned, then catch it with a control that never asks the model. | ~4-6 min | 100 min |
