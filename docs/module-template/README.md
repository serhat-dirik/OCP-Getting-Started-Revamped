...
Extracted from the Platform Orientation vertical slice (2026-07-08). Copy `skeleton/` and follow the loop in 03-DEV-WORKFLOW §3: **perform the lab on the cluster first, write second**. Every command block shows output you actually captured; every timing is measured.

Every module is identified by its **slug** — the directory/file name across all artifacts below (no number in the path). A module's **number is its position in `/modules.yaml`**, and the displayed order lives in the `nav-*.adoc` sidebars; the two are decoupled (owner decision 2026-07-17), so registering a new module = add a `{slug, title}` entry to `modules.yaml` at the right position **and** add its nav lines (`tools/check-module-order.sh` asserts the two agree). Cross-reference other modules by **name** (`xref:<slug>/…[Name]`), never by number.
...
| Slides outline | `slides/outlines/<slug>.md` | 5–8 slides; validate with `tools/slides/build-deck.py` |
| Media manifest | `content/modules/ROOT/assets/images/<slug>/media-manifest.md` | every screenshot spec'd; asciinema script; embeds marked `// media-pass:` (never `TODO` — the media pass is a planned phase, not an open item; DoD's zero-TODO check must stay meaningful) |
...
## Pattern-locking rules (Platform Orientation lessons — follow or your build breaks)
...
8. **Entry chart conventions:** marker ConfigMap `ws-entry-<slug>` in the primary namespace (verify scripts check it); `ws-meta.yaml` lists purge namespaces for reset; every template carries a one-line "why" comment; secrets only by reference. **Optional `smokeCommands:` key** — a `ws-meta.yaml` list of 1–3 representative attendee commands the module genuinely leans on (an in-terminal `mvn …`, an `argocd app delete`); `ws smoke <slug>` runs each as `{user}` inside the live cockpit and asserts exit 0 (`${USER}` resolves to the target user). Default empty — the generic smoke battery already covers the cockpit/identity class, so add only load-bearing commands (YAGNI). Example: `smokeCommands:` then `  - "oc get deployment claims-analysis -n ${USER}-prod"`.
...
G1 additionally requires the **attendee-cockpit gate**: `ws smoke <slug> [userN]` green — it runs the attendee path (ws-on-PATH, `$HOME`/JVM `user.home`, isolation, freshness, in-cockpit `ws prep`/`verify`, plus any `smokeCommands`) from inside the live cockpit as `{user}`, never the admin kubeconfig. Paste its output as G1 evidence. Runs from admin/builder context after you've materialized the module (`ws prep <slug> --user userN`, then `ws git-refresh --restart-terminals --user userN` so the cockpit clone is fresh).
14. **Verify scripts are MODE-SPLIT: entry checks never run at completion.** Checks that assert
    ...
    Earned twice on 2026-07-11: gitops-fundamentals's verify hardcoded stage==2 against a lab that ends at 3, and
    trusted-supply-chain's "seed-vulnerable carries the CVE" entry check fired red after the attendee had correctly
