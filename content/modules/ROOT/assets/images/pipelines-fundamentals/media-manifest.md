...
Update 2026-08-01: #1 re-shot and re-embedded, so every console row in this module is now
captured except the Gitea webhook form (#2), which needs a human. The re-shoot was taken as
**user2**, not user1 — an owner decision, and the reason is worth keeping: `user1-cicd` was
holding `trusted-supply-chain`'s signed ImageStream while that module's own row 1 was still
uncaptured, and the two modules evict each other, so shooting here as user1 would have
destroyed an unshot state for cosmetic consistency. This frame therefore reads `user2-cicd`
beside four neighbours reading `user1-cicd`. Do not "fix" that by rebuilding user1.
...
|---|----------|--------|------|--------|-------------|
| 1 | `pipelines-fundamentals-01-pipelinerun-graph.png` | ✅ CAPTURED 2026-08-01 (replaces the 2026-07-26 asset) — shot as **user2** in `user2-cicd`, against a green `solve-parasol-claims-wdr4w`. Verified by eye, not by the run log: the five-Task DAG renders, every node green-ticked at `1/1`, the run badged **Succeeded**, duration 10m22s, and the full tab row Details / YAML / Parameters / Logs / Events / Output / TaskRuns is present — which is what proves `pipelines-console-plugin` mounted this time. **The caption changed with the frame.** `ws solve` produces a *finished* run, so the alt text written for a *running* one ("per-node step counters filling in") would have described something the picture does not show; it now describes the finished graph and tells the attendee that an unstarted Task reads `0/1`. The old caption's "Conditions table reporting Tasks Completed" was dropped outright: this console's PipelineRun Details page has no such table — the right-hand column carries Status, Vulnerabilities, Pipeline, Start/Completion time, Duration and Timeouts. Job: `tools/media/jobs-pipelines-fundamentals.yaml` (its only `reshoot: true`; **take that flag off now the good shot is on disk**). Note `tools/media/jobs-first-block.yaml` carries a second, user1-pointed copy of this same job, also flagged `reshoot: true` — running it would rebuild `user1-cicd` and clobber this frame. | **Pipelines → PipelineRuns → (your run)** — the 5-Task graph (fetch-source → unit-test → build-image → image-report → deploy) | the visual DAG; a green run; the tab row that only the Tekton console plugin draws | lab.adoc ex. 1, after the Console/CLI log-streaming tabs |
