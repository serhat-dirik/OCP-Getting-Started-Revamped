...
| 2 | `serverless-zero-to-hero-02-topology-scaled-up.png` | ⬜ NOT CAPTURED 2026-08-01 — **blocked on console authentication** (see the note below). Topology is stock console, so this one is purely a login away; the module's own timing caveat still applies when it is shot: drive the burst and watch for the ring to *reach* the ceiling rather than shooting on a timer | Circle: the pod ring filled to the **max-scale=3** ceiling | lab.adoc ex. 3 (autoscale-under-load beat) |
...
| 4 | `serverless-zero-to-hero-04-serverless-eventing.png` | ⬜ NOT CAPTURED 2026-08-01 — **two blockers, and the second is the interesting one.** (a) No console session (note below). (b) **Cluster 2 has no Knative console plugin**: `oc get consoleplugins` returns ten and none is Knative/Serverless, and the console operator's enabled plugin list has no entry for one — so the **Serverless** sidebar item, and with it `Serverless → Eventing`, **does not exist on that cluster**, even though Serverless 1.37.1 / Knative Serving 1.17 / Knative Eventing are installed and serving traffic. This does not contradict the `[CAPTURE-VERIFY]` entries below, which were confirmed live on a *different* cluster on 2026-07-31; it means the two clusters differ, which is a platform-provisioning question rather than a content defect. Flagged to the PM | Circle: the **Trigger → parasol-claims** subscriber edge | lab.adoc ex. 5 (eventing-taste beat) |

### Capture note, 2026-08-01

**Console rows are blocked on an OAuth session.** Establishing one requires a human to type a
password into the OpenShift login page — `tools/media/login.py` exists precisely because nothing
else can do it — and the operator of this pass is barred from typing credentials anywhere. The
cached `tools/media/shot-profile.session.json` holds a valid session for a *different* cluster; for
cluster 2 it carries only an unfinished `login-state`, and a headless probe of that console
returned **401**. Row 2 is one human login away. Row 4 additionally needs a cluster whose Serverless
install registers its console plugin.
