...
|----------|--------|-------|
| `m14-multi-tenancy-workload-security-01-identity-flow.svg` | concept.adoc Mermaid "identity → authority" | IdP → User → **Group** → RoleBinding → Role → verbs; highlight the group as the design lever; reused on slide 2 |
| `m14-multi-tenancy-workload-security-02-scc-admission.svg` | concept.adoc Mermaid "restricted-v2 admission" | Pod `runAsUser:0` → restricted-v2 UID-range gate → **rejected** vs **fix the image** vs **scoped SCC grant**; the module's anchor diagram, reused on slide 4 |
| `m14-multi-tenancy-workload-security-03-platform-accretion.svg` | concept.adoc — media-pass pending (centrally maintained master diagram) | **master accretion diagram**, the M14 layer (tenant sandbox: RBAC + quota + SCC around `{user}-dev/stage/prod`) highlighted on the running Parasol platform |
| `m14-multi-tenancy-workload-security-04-what-you-built.svg` | wrapup.adoc Mermaid recap | tenant sandbox (payments-ci/ops, non-root workload, quota) vs platform-owned levers (platform-observer, self-provisioning/template, OAuth IdPs); green = tenant self-service, blue = platform-owned |
...
### Terminal cast — the five-minute safe sandbox (`m14-multi-tenancy-workload-security-demo.cast`, ~10 min, MANDATORY)
...
|---|----------|------|----------|-------------|
| 1 | `m14-multi-tenancy-workload-security-01-deploy-0of1-event.png` | Console → Workloads → Deployments → `root-demander` → Events tab, showing the SCC `FailedCreate` | Circle: `0 of 1` pods + the `restricted-v2 … runAsUser` event line | lab.adoc ex. 1 Console tab |
| 2 | `m14-multi-tenancy-workload-security-02-create-rolebinding.png` | Console → User Management → RoleBindings → Create binding form (subject=ServiceAccount payments-ci, role=edit) | Circle: Subject type=ServiceAccount, Role name=edit | lab.adoc ex. 3 Console tab |
| 3 | `m14-multi-tenancy-workload-security-03-resourcequota-gauges.png` | Console → Administration → ResourceQuotas → `workshop-quota` donut gauges | Circle: requests.memory used-vs-hard (6Gi cap) | lab.adoc ex. 5 Console tab |

**Animated gif (PREFERRED for the multi-click RoleBinding flow):**
`m14-multi-tenancy-workload-security-04-grant-role.gif` (<30 s, silent) — the Console path of ex. 3:
