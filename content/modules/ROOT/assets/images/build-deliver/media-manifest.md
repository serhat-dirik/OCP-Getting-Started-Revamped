...
| 4 | `build-deliver-04-topology-built-and-wired.png` | ❌ RE-CAPTURE | Topology, project `user1-dev`, claims + claims-db + notifications all healthy | the `parasol-claims`→`claims-db` connection; the Open-URL arrow on claims. RE-CAPTURE: pulled 2026-07-28 — the captured asset on disk shows only 2 nodes, ungrouped, no `parasol-notifications` and no visible connector; it contradicts the grouped-topology step and was already un-embedded in `lab.adoc` (see the `// media-pass:` note near exercise 5). | lab.adoc ex. 5 (end state) |
