...
| 3 | `build-deliver-03-buildconfig-to-imagestream.png` | ⬜ NOT CAPTURED — **refused, 2026-08-01, and re-confirmed against the entry state rather than inherited.** No materialisable state carries a BuildConfig: the entry state is an empty dev namespace by design, and `ws solve build-deliver` deploys a **prebuilt** image, so the completed Build and the S2I ImageStream tag this row wants exist only after a human performs exercise 1's import by hand. It also has no `// media-pass:` embed point in `lab.adoc`, so nothing on the page is waiting for it. Parked with this reason in `tools/media/jobs-first-block.yaml`; same conclusion as `RUNBOOK.md`. | Topology `parasol-claims` node → **Resources**: the completed Build and the ImageStream tag | the `Complete` build; `parasol-claims:latest`; the "Source (S2I)" strategy | lab.adoc ex. 1 (inspect) |
| 4 | `build-deliver-04-topology-built-and-wired.png` | ✅ DONE 2026-07-31 | Topology, project `user1-dev`, claims + claims-db + notifications all healthy, grouped under one `parasol` application | the `parasol-claims`→`claims-db` connection (arrow, visible). RE-CAPTURED 2026-07-31 as user1 via `ws solve build-deliver --user user1`: all three workloads 1/1 and grouped (`app.kubernetes.io/part-of=parasol`), connector present. **The Open-URL arrow is NOT in this capture** — verified live (not a capture miss): the Route is correctly wired (`oc get route` shows it; the console's own side panel → Resources → Routes lists it with a working Location URL), and the Display-options menu was checked exhaustively (`Expand`, `Application groupings`, `Show > Pod count/Labels` — no decorator toggle exists). The decorator simply does not render on a node that belongs to a multi-member application group in this console build; it does render on a lone ungrouped node (see `platform-orientation-01-topology-3pods.png`, captured on the same cluster). Content question, not a capture defect — flagging for a manifest/lab-text decision rather than re-shooting again. `parasol-notifications`'s on-screen label is truncated to `paraso…ations` by the console's fixed node-label width; confirmed via the node's `data-test-id` attribute that it is the right workload. | lab.adoc ex. 5 (end state) |
