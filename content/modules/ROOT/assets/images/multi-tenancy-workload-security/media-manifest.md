...
|---|----------|------|----------|-------------|
| 1 | `multi-tenancy-workload-security-01-deploy-0of1-event.png` | ✅ CAPTURED 2026-08-01 as **user7**, after the first attempt failed on `never saw 'FailedCreate'` against a page that was displaying the refusal perfectly. **The console's Events tab does not render an event's REASON** — grounded: an entry shows the involved object, the namespace, a *Generated from replicaset-controller* line, an occurrence count and the message, and nothing else. `FailedCreate` is the CLI's REASON column, not a string this view has. The waits now name the message text (`security context constraint`, `runAsUser`, `restricted-v2`), which is also what the exercise asks the attendee to read. Everything this row predicted held: the event really is on the ReplicaSet rather than the Deployment, and it really does name restricted-v2 rejecting `runAsUser: 0` against the range 1001990000-1001999999. **The row does NOT need splitting after all** — the caption's `0 of 1` belongs to the Deployment page the lab has just told the attendee to come from, and the alt text now says so instead of promising it in this frame. | Circle: `0 of 1` pods + the `restricted-v2 … runAsUser` event line | lab.adoc ex. 1 Console tab |
