...
## Slide: Zero-downtime — capacity AND data

- maxUnavailable: 0 — surge before retire, capacity flat
- Recreate — for single-writers (deliberate gap)
- The real trap: a pod that *rebuilds shared state on boot*
- Fix: schema-management=none + seed once at deploy + raise the CPU limit
- A PDB makes a node drain wait its turn

Notes: A Deployment doesn't just run pods, it replaces them — and how it replaces them has a consequence. RollingUpdate with maxUnavailable 0 is the load-bearing capacity setting: it surges a new pod to Ready before retiring an old one, so ready-pod count never dips (you watch it hold flat through a roll). Recreate is correct for a single-writer database — a deliberate gap so two copies never run at once. But capacity is only half the story. Parasol's claims API shares one PostgreSQL, and the image boots Hibernate with drop-and-create — so every pod boot drops and reseeds the shared database. During a rolling update the new pod wipes the tables out from under the old pods still serving, and a claim filed since the last deploy is silently gone. maxUnavailable 0 did its job — three ready pods throughout — and the data still vanished, because the disruption is in the data plane at startup, not the connection plane at shutdown; no preStop or SIGTERM handling touches it. The fix stops pods rebuilding shared state on boot: schema-management=none, seed the database once at deploy (a Flyway/Liquibase Job), and raise the CPU limit so the cold-start is quick. In the lab you prove it deterministically — plant a value, restart one pod, watch it revert to the seed — then fix it and watch the value survive. Finally a PodDisruptionBudget protects voluntary disruptions: a node drain must respect minAvailable, so it waits its turn instead of taking your last replica — and minAvailable equal to the replica count deadlocks the drain.
Visual: Two panels — top "maxUnavailable:0" with a flat "3/3 ready" line through a rollout; bottom a new pod's "drop-and-create on boot" arrow wiping a shared database while an old pod still reads it, labelled "data plane, at startup."
