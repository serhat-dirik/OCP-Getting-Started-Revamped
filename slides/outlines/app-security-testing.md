...
Visual: The concept diagram app-security-testing-01-five-pillars.svg — source → SAST → SCA → build → image scan → config check → deploy → DAST, with the five gates coloured by family (shift-left blue, shift-right amber) and the plain build/deploy steps in grey. Reused on concept slide 3.
...
Visual: The same app-security-testing-01-five-pillars.svg, read this time as two bands — the blue shift-LEFT gates sitting before the "build" step, the amber shift-RIGHT gates after it — with a cost arrow underneath rising from "cheap one-line fix" on the left to "production incident" on the right.
...
- SAST red → externalize the hard-coded secret (Config, Secrets & Multi-Environment's pattern) → green, and the next gate fires
