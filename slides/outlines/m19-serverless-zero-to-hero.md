...
Visual: Reuse the concept diagram m19-...-01-request-driven-compute.svg — the Configuration minting revisions, the Route splitting weight over v1 (#stable) and v2 (#candidate), the KPA scaling each 0..N.
...
- Third traffic-shift tool: Argo Rollouts (M10), service mesh (M18), Knative revisions (here)
...
Visual: The ksvc Route fanning to a fat 80% arrow (v1 #stable) and a thin 20% arrow (v2 #candidate), each tag with its own sub-route chip; a small "rollback: stable=100 (one line)" callout; a footnote comparing M10 Rollouts / M18 mesh / here.
...
- Wiring + scale-up are REAL; processing the event is M20
...
Visual: PingSource → Broker (in-memory) → Trigger → ksvc, with a lightning bolt on the ksvc "wakes from zero" and a callout on the delivery edge "HTTP 404 — no handler yet → M20"; the whole panel tinted as a "taste" with a forward-pointer to the eventing module.
...
Visual: A two-column decision matrix (Lean serverless | Lean Deployment) across five rows (traffic shape, latency budget, invocation, rollout need, cost model), with a footnote pointer to M20 (eventing depth + SonataFlow workflows) and M06 (batch/Jobs) for "not this tool."
