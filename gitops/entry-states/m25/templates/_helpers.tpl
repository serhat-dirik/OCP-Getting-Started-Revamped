{{/* The module namespace — the per-user dev namespace this chart materializes INTO but does NOT own
     ...
     like M02). {user}-dev is SHARED with the other inner-loop modules → conflictsWith them (ws-meta). */}}
{{- define "m25.namespace" -}}{{ .Values.user }}-dev{{- end -}}
...
{{- define "m25.notificationsRepoUrl" -}}https://gitea-{{ .Values.giteaNamespace }}.{{ .Values.clusterDomain }}/{{ .Values.user }}/{{ .Values.forkRepo }}{{- end -}}
...
{{- define "m25.imageRef" -}}image-registry.openshift-image-registry.svc:5000/{{ .Values.user }}-dev/{{ .Values.imageName }}:{{ .Values.imageTag }}{{- end -}}
