...
7. **Health that stays Degraded/OutOfSync on purpose must say so in THREE places.** A module whose entry state is intentionally unhealthy at rest (a seeded fault the attendee is meant to diagnose/fix — currently only `ai-assisted-development`) declares `expectDegraded: true` in `ws-meta.yaml`; `ws start`/`ws solve`/`ws verify` read it from the repo. That alone is invisible to anything with ONLY API-server access (a monitor, a CI health probe) — so `ws`'s `render_app` also stamps the live Application with `workshop.redhat.com/degraded-by-design: "true"` whenever `expectDegraded` is true (never written when false/absent). `ws doctor` and `ws status` read that label straight off the object (falling back to the ws-meta.yaml read only for an app materialized before the label existed) so a genuinely broken module is never mistaken for one that is Degraded by design, or vice versa. A module that is Healthy but legitimately never `Synced` (a server-defaulted field Argo diffs forever) is a different case — `expectOutOfSync: true` — and does NOT get this label.

   **The third place is the RESOURCE itself, and it is the one that was missing.** Both of the above live on the *Application*, and nobody reaches a broken pod through the Application: they reach it through `oc get pods -n {user}-dev` or the console. On 2026-08-13 `entry-ai-assisted-development-user6/user7` read Degraded and `entry-agentic-ai-user1/user2` read OutOfSync, both correctly, and the project lead began debugging the seeded fault before catching it. So every deliberately-broken or deliberately-drifting object now carries its own explanation:

   | key | kind | value |
   |---|---|---|
   | `workshop.redhat.com/by-design` | label | `"true"` — the ONE selector, `oc get <kinds> -A -l workshop.redhat.com/by-design=true` |
   | `workshop.redhat.com/by-design-symptom` | annotation | the short thing an operator actually SEES (`Degraded — pod Running 0/1, …`) |
   | `workshop.redhat.com/by-design-reason` | annotation | why it is correct, and what NOT to do about it |

   Shape rationale: the label mirrors `degraded-by-design`'s boolean form (stamped only when true; absence means normal) and stays a single closed key on purpose — Kubernetes cannot select on a label-key *prefix*, so an open set of keys (`degraded-by-design`, `outofsync-by-design`, …) would force `ws by-design` to carry a hardcoded key list that rots silently. The prose cannot live in the label because a label value is capped at 63 characters and rejects spaces; an annotation cannot be selected on. Hence one selectable label plus two annotations.

   Gate the marker on `.Values.solve` when the object is only actually broken at entry (`ai-assisted-development`'s probe, `multi-tenancy-workload-security`'s root-demander) — a fault marker left on a healthy object is noise that teaches people to ignore markers. Leave it ungated when it describes a permanent property of the object rather than a defect solve repairs (`agentic-ai`'s attendee-owned ConfigMap, `gitops-at-scale`'s failure switch, `observability-health-scale`'s load generator, `trusted-supply-chain`'s Pipeline).

   `ws by-design` reads all three signals back in one command and is the supported answer to "is this red expected?". If you mark a resource whose *kind* it does not sweep, it says so — but fix `BY_DESIGN_KINDS` in `tools/ws/ws` rather than leaving it saying so, because a marked object missing from that report reads as "not expected", which is the exact false negative the marker exists to prevent.
8. **Workshop-built images need `imagePullPolicy: Always`.** Anything under `ogsr-parasol-images/` (and per-user istags this repo builds) is tagged `1.0`/`1.1` and **rebuilt in place**, so the tag is mutable. Kubernetes defaults a non-`:latest` tag to `IfNotPresent`, and a node that has already cached that tag will happily keep serving the old layers — measured on a live cluster, 2026-07-29: all three `parasol-claims` tags moved to a new digest, and `oc rollout restart` brought the *old* digest back up. Leave upstream images alone: `:latest` already defaults to `Always`, and anything pinned to a digest or a real version tag (`postgresql:15-el9`, `udi-rhel9:3.29`) should stay `IfNotPresent`. Knative Services are the one exception — Knative resolves the tag to a digest when it creates the Revision, so setting `Always` there only costs a registry round-trip on every scale-from-zero; a rebuild reaches a ksvc through `ws reset` (new Revision), not a pod restart. **Check `_helpers.tpl` too** — a named template that renders a PodSpec is invisible to a `grep` over `templates/*.yaml`, and that is exactly how `config-multienv` was missed twice (this sweep, and the Route-TLS sweep of 2026-07-27).
