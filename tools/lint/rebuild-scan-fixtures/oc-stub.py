...
indistinguishable from "nothing there" — the exact reason `ws doctor`'s drift row has to gate on
`oc auth can-i` before scanning. A stub that quietly returned nothing for a call it did not model
