#!/usr/bin/env bash
# Verify M24 — AI-Assisted Development on OpenShift (vibe coding, safely).
...
seed_probe_is_bad() {
  local want got
  want="$(oc get cm ws-entry-m24 -n "$NS" -o jsonpath='{.data.badProbePath}' 2>/dev/null || true)"
...
check "namespace ${NS} exists"                       oc get ns "$NS"                        || hint "run: ws prep m24 (or ws start m24 --user ${USER_NAME}); ${NS} is workshop-layer (per-user-namespaces)"
check "entry marker ws-entry-m24 present"            oc get cm ws-entry-m24 -n "$NS"        || hint "entry app not synced — ws reset m24 --user ${USER_NAME}"
check "scoped ServiceAccount mcp-agent present"      oc get sa "$SA" -n "$NS"               || hint "the MCP server's least-privilege SA is missing — ws reset m24 --user ${USER_NAME}"
check "mcp-agent read-only RoleBinding (view) present" oc get rolebinding "${SA}-view" -n "$NS" || hint "the namespaced view grant is missing — ws reset m24 --user ${USER_NAME}"
check "MCP server deployed + Ready"                  deploy_available kubernetes-mcp-server || hint "kubernetes-mcp-server not Ready — check pods in ${NS} (ws reset m24 --user ${USER_NAME}); pulls the digest-pinned ghcr.io/containers/kubernetes-mcp-server"
check "MaaS config present (endpoint + model)"       oc get cm maas-config -n "$NS"         || hint "entry app not synced — ws reset m24 --user ${USER_NAME}"
check "MaaS credentials Secret present"              oc get secret maas-credentials -n "$NS" || hint "the MaaS copy hook did not run — ws reset m24 --user ${USER_NAME}"
check "Dev Spaces workspace present"                 oc get devworkspaces.workspace.devfile.io "$(oc get cm ws-entry-m24 -n "$NS" -o jsonpath='{.data.devWorkspaceName}' 2>/dev/null || echo parasol-ai-assist)" -n "$NS" \
  || hint "the DevWorkspace is missing — ws reset m24 --user ${USER_NAME}"

# INFO: the pinned MCP image + converged model (proves digest-pin + per-cluster secret-sourcing).
info "MCP server image: $(oc get cm ws-entry-m24 -n "$NS" -o jsonpath='{.data.mcpServerImage}' 2>/dev/null || echo '?')"
...
if [[ "$ENTRY_ONLY" == "true" ]]; then
  ...
  check "seeded parasol-claims is BROKEN (Running 0/1, readiness failing)" seed_broken \
    || hint "the diagnosis target should be 0/1 at entry; if it is Ready the seed did not materialize with the fault — ws reset m24 --user ${USER_NAME}"
  check "the injected fault is the wrong readinessProbe path"              seed_probe_is_bad \
    || hint "the seed's readinessProbe path is not the marker's badProbePath — ws reset m24 --user ${USER_NAME}"
  check "scoped write NOT yet granted to mcp-agent (read-only pre-lab)"    scoped_write_absent \
    || hint "an edit/admin RoleBinding for mcp-agent already exists — entry state should be read-only; ws reset m24 --user ${USER_NAME}"
else
  ...
  check "seeded parasol-claims FIXED + Ready (probe patched to the correct path)" deploy_available "$SEED" \
    || hint "parasol-claims is still 0/1 — the readinessProbe path was not fixed; ws solve m24 --user ${USER_NAME} (or let the agent patch it), then retry"
fi
