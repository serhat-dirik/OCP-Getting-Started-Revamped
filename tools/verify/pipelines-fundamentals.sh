...
check "namespace ${NS} exists"                            oc get ns "$NS"                                    || hint "run: ws prep pipelines-fundamentals (or ws start pipelines-fundamentals --user ${USER_NAME})"
check "entry marker ws-entry-pipelines-fundamentals present"                 oc get cm ws-entry-pipelines-fundamentals -n "$NS"                    || hint "entry app not synced — ws prep pipelines-fundamentals (or ws start pipelines-fundamentals --user ${USER_NAME})"
...
check "Pipeline parasol-claims-build-test-deploy present" oc get pipelines.tekton.dev parasol-claims-build-test-deploy -n "$NS" || hint "entry app not synced — ws prep pipelines-fundamentals (or ws start pipelines-fundamentals --user ${USER_NAME})"
...
if [[ "$ENTRY_ONLY" == "true" ]]; then
  ...
  check "parasol-claims deployment ready in ${NS}"        deploy_ready parasol-claims "$NS"                  || hint "not done yet? the pipeline is what deploys and wires this app to claims-db, so before you run it this red is expected, not a broken environment (or: adm solve pipelines-fundamentals --user ${USER_NAME}). If a run already Succeeded and the app is still not ready, that one is real: oc get pods -n ${NS}; tkn pr list -n ${NS}"
  ...
  check "parasol-claims Route created by the pipeline in ${NS}" oc get route parasol-claims -n "$NS"         || hint "not done yet — the deploy step creates the edge Route itself, so it appears only once the pipeline has run (or: adm solve pipelines-fundamentals --user ${USER_NAME}); attendees never run oc expose here"
fi
