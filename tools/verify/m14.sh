#!/usr/bin/env bash
# Verify M14 — Multi-Tenancy & Workload Security.
...
# so they AUTO-SKIP unless the caller can impersonate (admin/CI) — mirroring m06's ClusterQueue skip.
...
check "namespace ${NS} exists"                          oc get ns "$NS"                              || hint "run: ws prep m14 (or ws start m14 --user ${USER_NAME})"
check "entry marker ws-entry-m14 present"               oc get cm ws-entry-m14 -n "$NS"              || hint "entry app not synced — ws reset m14 --user ${USER_NAME}"
check "teammate SA payments-ci present"                 sa_exists payments-ci                        || hint "entry app not synced — ws reset m14 --user ${USER_NAME}"
check "teammate SA payments-ops present"                sa_exists payments-ops                       || hint "entry app not synced — ws reset m14 --user ${USER_NAME}"
check "workload SA root-demander present"               sa_exists root-demander                      || hint "entry app not synced — ws reset m14 --user ${USER_NAME}"
...
check "root-demander Deployment present"                oc get deploy root-demander -n "$NS"         || hint "entry app not synced — ws reset m14 --user ${USER_NAME}"

if [[ "$ENTRY_ONLY" == "true" ]]; then
  # --- entry state: the workload has NOT been run and the teammates are UNGOVERNED -----------------
  check "root-demander is not running yet (scaled to 0)" deploy_idle root-demander                   || hint "entry ships it at 0 replicas; if it is running, the lab already started — ws reset m14 --user ${USER_NAME}"
  if [[ "$IMPERSONATE_OK" == "true" ]]; then
    ...
    check "payments-ci is ungoverned in ${NS} (no edit yet)"   sa_cannot payments-ci "$NS" update deployments   || hint "a leftover RoleBinding governs payments-ci — remove lab-created bindings, then ws reset m14 --user ${USER_NAME}"
    check "payments-ops is ungoverned in ${NS} (no deployer yet)" sa_cannot payments-ops "$NS" create deployments || hint "a leftover RoleBinding governs payments-ops — remove lab-created bindings, then ws reset m14 --user ${USER_NAME}"
  fi
else
  ...
  check "root-demander now runs non-root (>=1 ready replica)" deploy_ready root-demander             || hint "fix the image (drop runAsUser:0) and scale up — or ws solve m14 --user ${USER_NAME}"
  if [[ "$IMPERSONATE_OK" == "true" ]]; then
