...
check "entry marker ws-entry-gitops-fundamentals in ${GITOPS}"           oc get cm ws-entry-gitops-fundamentals -n "$GITOPS"                 || hint "entry app not synced — ws prep gitops-fundamentals (or ws start gitops-fundamentals --user ${USER_NAME})"
...
if [[ "$ENTRY_ONLY" == "true" ]]; then
  ...
  check "parasol-claims ready in ${DEV}"                 deploy_ready parasol-claims "$DEV"                  || hint "not done yet? creating your Argo Application (overlays/dev) on student-gitops IS the lab, so this red is expected before you do it (adm solve gitops-fundamentals --user ${USER_NAME} creates it for you). If the Application exists and is stuck OutOfSync with a permissions error, that one is real — the platform enrollment (managed-by) may be missing; see the module's troubleshooting page"
